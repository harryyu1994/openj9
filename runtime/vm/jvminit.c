...
static UDATA checkArgsConsumed(J9JavaVM * vm, J9PortLibrary* portLibrary, J9VMInitArgs* j9vm_args) {
	...
	if (!ignoreUnrecongizedXXColonOptions && !vm->sharedCacheAPI->xShareClassesPresent) {
		...
		findArgInVMArgs( PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXSHARECLASSESPORTABLE, NULL, TRUE);
		findArgInVMArgs( PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXNOSHARECLASSESPORTABLE, NULL, TRUE);
	}
